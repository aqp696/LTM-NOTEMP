...
static int do_receiver_part(t_direccion *dir_local, t_direccion *dir_remota) {
  ...
  do {
    ...
    len = t_receive(cx1, buf, bs, &flags);
    if (len < 0) {
    ...
    /* 
     * Transmitimos todos os datos recibidos. Se o outro extremo pechou, tamén pechamos
     * este sentido da comunación. 
     */
    len = t_send(cx1, buf, bs, &flags);
    if (len < 0 ) {
...
static int do_sender_part(t_direccion *dir_local, const t_direccion *dir_remota, size_t len) {
  ...
  do {
    ...
    if (len > 0 && (flags & CLOSE)) {
      fprintf(stderr, "\nFluxo de entrada pechado. Non deberían quedar datos \npendentes de lectura, pero quedan %zd bytes\n", len);
      len1 = EXCLOSE;
      break;
    }
